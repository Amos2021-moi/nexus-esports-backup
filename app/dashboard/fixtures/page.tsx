  ...
  switch (status) {
    ...
    case "REGISTRATION":
      return {
        ...
        canSuggestTime: false,
        message:
          "Registration is open. Fixtures will be generated after registration closes.",
        icon: Calendar,
      };
    case "FIXTURE_LOCK":
      return {
        ...
        canSuggestTime: true,
        message:
          "Fixtures are locked. You can view your matches and contact opponents.",
        icon: Lock,
      };
    case "LIVE":
      return {
...
export default function FixturesPage() {
  ...
  async function fetchCalendarSyncPreference() {
    try {
      const res = await fetch(
        "/api/settings?category=competition&key=fixtureCalendarSync",
      );
      if (res.ok) {
        const data = await res.json();
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-2xl border border-white/10 bg-gray-800/40 p-8 text-center backdrop-blur-xl">
          ...
          <p className="text-gray-400">
            You need to pay the entry fee to view your fixtures.
          </p>
          <p className="mt-1 text-sm text-gray-500">
            Entry Fee: KES {paymentInfo.entryFee}
          </p>
          <Link
            href="/dashboard"
        ...
        <div className="rounded-2xl border border-white/10 bg-gray-800/40 p-8 text-center backdrop-blur-xl">
          ...
          {seasonStatus === "REGISTRATION" && (
            <p className="mt-2 text-sm text-gray-500">
              Check back after the admin generates fixtures.
            </p>
          )}
        </div>
      </div>
  ...
  const getOpponent = (fixture: Fixture) => {
    ...
    const opponentName =
      opponent?.profile?.username || opponent?.name || "Opponent";
              ...
              >
                ...
                {isPending && fixture.result?.evidenceImage && (
                  <div className="mt-3 text-center">
                    <EvidenceViewer
                      evidenceImage={fixture.result.evidenceImage}
                    />
                  </div>
                )}

                {/* Locked Indicator for PENDING */}
                {isPending && (
                  <div className="mt-3 text-center">
                ...
                {/* Match Center Link - For completed or pending fixtures */}
                {!isActionable && (
