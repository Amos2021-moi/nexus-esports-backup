...
function DecorBackground() {
  return (
    <div className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
      ...
      <div className="absolute -top-32 -left-24 h-80 w-80 rounded-full bg-indigo-600/25 blur-[120px]" />
      <div className="absolute top-1/3 -right-24 h-96 w-96 rounded-full bg-blue-600/20 blur-[120px]" />
...
export default function PrivacySettingsPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSettings({
          ...
          allowComments:
            data.allowComments !== undefined ? data.allowComments : defaultSettings.allowComments,
          publicProfile:
            data.publicProfile !== undefined ? data.publicProfile : defaultSettings.publicProfile,
          showLastSeen:
            data.showLastSeen !== undefined ? data.showLastSeen : defaultSettings.showLastSeen,
        })
      }
    } catch (error) {
  ...
  // ----- Grouped option definitions (UI only — keys untouched) -----
  ...
  const contentOptions = [
    {
      ...
      description: "Display your match stats to others",
      icon: Eye,
    },
    {
      key: "showSquad" as keyof PrivacySettings,
      label: "Show Squad",
      description: "Let others see your squad on your profile and match pages",
      icon: Users,
    },
    {
      key: "allowComments" as keyof PrivacySettings,
      label: "Allow Comments",
      description: "Enable comments from other players on your posts",
      icon: MessageSquare,
    },
  ]

  // ----- Privacy level summary (updates in real time) -----
  const enabledCount = Object.values(settings).filter(Boolean).length
  const totalCount = Object.values(settings).length

  let privacyLevel: { label: string; tone: string; ring: string; dot: string; icon: typeof Globe }
      ...
      <>
        ...
        <div className="flex items-center justify-center min-h-[60vh]">
          <div className="text-center">
            ...
            <p className="text-gray-400 text-sm">Loading privacy settings…</p>
          </div>
        </div>
      </>
    )
  ...
  const ToggleRow = ({
    ...
    option: { key: keyof PrivacySettings; label: string; description: string; icon: typeof Eye }
  }) => {
        ...
        >
          ...
          <div className="relative flex items-start gap-3">
            <div
              className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl transition-colors ${
                isEnabled
                  ? "bg-gradient-to-br from-indigo-500/30 to-blue-500/20 text-indigo-300"
          ...
          <button
            ...
            className={`relative inline-flex h-7 w-12 shrink-0 items-center rounded-full p-0.5 transition-colors duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400/60 ${
              isEnabled
                ? "bg-gradient-to-r from-indigo-500 to-blue-500 shadow-[0_0_16px_-2px_rgba(99,102,241,0.6)]"
                : "bg-gray-700"
            }`}
          >
            <motion.span
  ...
  const SectionCard = ({
    ...
    <motion.section
      ...
      className="relative overflow-hidden rounded-3xl border border-white/10 bg-gray-800/40 p-5 shadow-2xl backdrop-blur-xl sm:p-6"
    >
      ...
      <div className="mx-auto max-w-3xl px-4 pb-32 pt-6 sm:pt-8">
        <motion.div initial="hidden" animate="visible" variants={containerVariants}>
          ...
          <motion.div variants={itemVariants} className="mb-6">
            <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-gray-800/40 p-6 shadow-2xl backdrop-blur-xl">
              ...
              <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                {/* Live privacy status badge */}
                <div
                  className={`flex items-center gap-2 self-start rounded-full border px-3.5 py-2 ${privacyLevel.ring}`}
                >
                  <span className="relative flex h-2.5 w-2.5">
                    <span
                      className={`absolute inline-flex h-full w-full animate-ping rounded-full opacity-60 ${privacyLevel.dot}`}
                    />
                  ...
                  <span className={`text-sm font-semibold ${privacyLevel.tone}`}>
                    Profile is {privacyLevel.label}
                  </span>
                </div>
              </div>
              ...
              <div className="relative mt-5 grid grid-cols-3 gap-3">
                ...
                <div className="rounded-2xl border border-white/10 bg-gray-900/40 p-3 text-center">
                  <p className="text-lg font-bold text-white">
                    {settings.publicProfile ? "Visible" : "Hidden"}
                  </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <SectionCard
              ...
              subtitle="Who can find and view your profile"
              accent="from-indigo-500 to-blue-500"
            >
              {visibilityOptions.map((option) => (
                <ToggleRow key={option.key} option={option} />
              ))}
            </SectionCard>
            ...
            <SectionCard
              icon={Sparkles}
              title="Stats & Content"
              subtitle="Control your stats, squad and interactions"
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="truncate font-semibold text-white">
                      {settings.publicProfile ? username : "Private User"}
                    </p>
                    <p className="text-xs text-gray-400">
                      {settings.showLastSeen ? "Active recently" : "Last seen hidden"}
                    </p>
                  </div>
                </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-white/10 bg-gray-900/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-4 px-4 py-3">
          <p className="hidden text-xs text-gray-400 sm:block">
            <span className="font-semibold text-white">{enabledCount}</span> of {totalCount}{" "}
            settings public
          </p>
          <motion.button
            type="submit"
...
function PreviewChip({ on, label }: { on: boolean; label: string }) {
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-medium ${
        on
          ? "border-emerald-400/25 bg-emerald-500/10 text-emerald-300"
          : "border-white/10 bg-gray-800/60 text-gray-500 line-through"
      }`}
    >
